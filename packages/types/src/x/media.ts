...
export type XMediaUrlPolicy = (url: string, role: MediaRole) => string | undefined
...
export function mapXPostMediaUrls(post: XPost, map: XMediaUrlPolicy): XPost {
  function mapBase(entry: XPostBase): XPostBase {
    ...
    if (entry.media) result.media = entry.media.flatMap((media): XPostMedia[] => {
      if (media.type === 'photo') {
        const url = map(media.url, 'img')
        return url ? [{ ...media, url }] : []
      }
      const next = { ...media, sources: media.sources.flatMap((source) => {
  ...
  return { ...post, ...mapBase(post), ...(post.quote ? { quote: mapBase(post.quote) } : {}) }
}
export function getXPostMediaUrls(post: XPost): Array<{ url: string; role: MediaRole }> {
  const result: Array<{ url: string; role: MediaRole }> = []
  mapXPostMediaUrls(post, (url, role) => { result.push({ url, role }); return url })
  return result
}
