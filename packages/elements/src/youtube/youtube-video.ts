...
import { parseYouTubeVideoSchema } from '@post-embed/schema'
...
export function useYouTubeVideo(
  host: HostElement,
  props: State<YouTubeVideoProps>,
): void {
  const { fetched, pending } = useFetch(host, props, 'YouTube video')

  useHostEffect(host, () => {
    host.dataset.postEmbed = 'youtube-video'
    ...
    const result = data == null ? undefined : parseYouTubeVideoSchema(data)
    ...
    const ref =
      result && !result.issues ? parseYouTubeUrl(result.value.url) : undefined
...
function renderFallback(pending: boolean): HTMLElement {
  return el(
    ...
    el(
      'div',
      { 'data-details': '' },
      el('span', { 'data-title': '' }, 'YouTube video'),
    ),
    el(
      'p',
      { 'data-body': '' },
      pending ? 'Loading this video…' : 'This video is unavailable.',
    ),
    pending
      ? null
      : el(
