...
export function createLabSamples(origin: string): LabSample[] {
  const asset = (file: string) => new URL(`/lab/${file}`, origin).href
  ...
  const photo = (
    ...
    height: number,
    alt: string,
  ): XPostMedia => ({ type: 'photo', url: asset(file), width, height, alt })
  const video = (
    poster: string,
    width: number,
    height: number,
    ...
    sources: [
      {
        type: 'video/mp4',
        bitrate: 256000,
        url: new URL('/media/motion.mp4', origin).href,
      },
    ],
  })
  let id = 1000
  ...
  return [
    ...
    },
    {
      ...
      post: post(
        kai,
        'The new inbox ships today. Every row is keyboard reachable.',
